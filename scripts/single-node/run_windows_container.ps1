...
foreach ($Container in $ContainerList) {
    $Results = "$ResultsDir/single-node/windows_${Container}.csv"
    "nodes,threads,time,max_memory,cpu_percentage" | Set-Content $Results

    foreach ($Threads in $ThreadsList) {
        Write-Host "------------------------------------------------------------"
        Write-Host "Starting test with $Threads threads ($Container)..."
        ...
        $LogFile = "$LogDir/single-node/windows_${Container}_${Threads}threads.log"
...
Write-Host "------------------------------------------------------------"
Write-Host "All tests have finished. Results in $ResultsDir/"
